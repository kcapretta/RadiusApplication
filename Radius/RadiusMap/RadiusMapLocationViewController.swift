...
class RadiusMapLocationViewController: UIViewController, MKMapViewDelegate {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "newView" {
            let userViewController = segue.destination as! ViewController
            let storyboard = UIStoryboard(name: "Main", bundle: nil)
            let nextSegue = storyboard.instantiateViewController(identifier: "ViewController")
            self.navigationController?.performSegue(withIdentifier: "newView", sender: Any?.self)
        }
    }
    
}

extension RadiusMapLocationViewController {
